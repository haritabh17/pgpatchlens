...
def pass_a_group(entry: dict, files: list[dict]) -> dict:
    prompt = f"""You are pgpatchlens, reviewing a PostgreSQL commitfest patch series.
...
def pass_b_explain(entry: dict, groups: list[dict], diff: str) -> list[dict]:
    ...
    prompt = f"""You are pgpatchlens, explaining a PostgreSQL patch series to a reviewer.
...
def pass_c_findings(entry: dict, diff: str) -> list[dict]:
    prompt = f"""You are pgpatchlens, doing a correctness review of a PostgreSQL patch series.
